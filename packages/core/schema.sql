...
CREATE TABLE agents (
    ...
    agent_type VARCHAR(16) NOT NULL CHECK (agent_type IN ('agent', 'vehicle', 'robot', 'sensor')),
...
CREATE INDEX idx_agents_type ON agents(agent_type) WHERE deleted_at IS NULL;

-- Index for creation time queries
CREATE INDEX idx_agents_created ON agents(created_at DESC) WHERE deleted_at IS NULL;
...
CREATE TABLE trust_events (
    ...
    amount_cents BIGINT DEFAULT NULL CHECK (amount_cents IS NULL OR amount_cents >= 0),
...
SELECT create_hypertable('trust_events', 'timestamp', chunk_time_interval => INTERVAL '1 day');
...
CREATE UNIQUE INDEX idx_scores_current ON trust_scores(agent_did) WHERE is_current = TRUE;
...
CREATE OR REPLACE FUNCTION invalidate_previous_scores()
RETURNS TRIGGER AS $$
BEGIN
    UPDATE trust_scores
...
CREATE TRIGGER trigger_invalidate_scores
    ...
    FOR EACH ROW
    EXECUTE FUNCTION invalidate_previous_scores();
...
FROM agents a
LEFT JOIN trust_scores ts ON a.did = ts.agent_did AND ts.is_current = TRUE
