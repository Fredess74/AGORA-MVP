...
export async function getAgentTransactionCount(agentDid) {
    const db = getDb();
    if (!db)
        return 0;
...
export async function recordTransaction(agentDid, amount, commission, metadata = {}) {
    const db = getDb();
    if (!db)
        return;
    const { data: listing } = await db
...
export async function updateAgentMetrics(agentDid, trustScore, latencyMs) {
    const db = getDb();
    if (!db)
        return;
    // Get current metrics
...
/** Log a usage event */
export async function logUsage(agentDid, action, latencyMs, metadata = {}) {
    const db = getDb();
    if (!db)
        return;
    const { data: listing } = await db
