...
export async function fetchNpmPackage(name) {
    const [meta, downloads] = await Promise.all([
        fetch(`https://registry.npmjs.org/${encodeURIComponent(name)}`).then(r => r.json()),
        fetch(`https://api.npmjs.org/downloads/point/last-month/${encodeURIComponent(name)}`).then(r => r.json()).catch(() => ({ downloads: 0 })),
    ]);
...
export async function fetchNpmDownloads(name) {
    const [weekly, monthly] = await Promise.all([
        fetch(`https://api.npmjs.org/downloads/point/last-week/${encodeURIComponent(name)}`).then(r => r.json()).catch(() => ({ downloads: 0 })),
        fetch(`https://api.npmjs.org/downloads/point/last-month/${encodeURIComponent(name)}`).then(r => r.json()).catch(() => ({ downloads: 0 })),
    ]);
    return { weekly: weekly.downloads || 0, monthly: monthly.downloads || 0 };
}
/** Fetch npm data for multiple packages in parallel */
export async function fetchNpmBulk(packages) {
...
//# sourceMappingURL=npm-hn.js.map