...
export function computeCapabilityMatch(taskType, taskDomain, agentTags, agentDescription) {
...
export function computeResponseTimeScore(latencyMs) {
...
export function computeExecutionScore(apiSuccesses, apiTotal, dataPoints, expectedDataPoints) {
...
export function computeHistoryScore(transactionCount) {
...
export class LiveTrustTracker {
    ...
    speedMode;
    components;
    constructor(sessionId, agentId, agentName, agentDid, speedMode) {
    ...
    async updateComponent(component, score, reason) {
        ...
        await emit('trust_component_update', this.sessionId, 'Agora Trust Engine', `${componentLabel(component)}: ${entry.score.toFixed(3)}`, reason, this.speedMode, {
            component,
            score: entry.score,
            weight: entry.weight,
    ...
    async checkIdentity(hasDid, zkVerifiable) {
        let score = 0;
        if (hasDid)
            score = 0.6;
        if (hasDid && zkVerifiable)
            score = 1.0;
        await this.updateComponent('identity', score, hasDid ? (zkVerifiable ? 'DID verified + ZK-proof confirmed' : 'DID verified, no ZK proof') : 'No DID registered');
    }
    /** Set capability match score */
    async checkCapability(taskType, taskDomain, agentTags, agentDescription) {
        const score = computeCapabilityMatch(taskType, taskDomain, agentTags, agentDescription);
        await this.updateComponent('capability_match', score, `Match: ${(score * 100).toFixed(0)}% — task "${taskType}" vs agent capabilities`);
    }
    /** Update response time score */
    async recordResponseTime(latencyMs) {
        const score = computeResponseTimeScore(latencyMs);
        await this.updateComponent('response_time', score, `Response in ${(latencyMs / 1000).toFixed(1)}s — ${score >= 0.7 ? 'fast' : score >= 0.4 ? 'acceptable' : 'slow'}`);
    }
    /** Update execution quality score */
    async recordExecution(apiSuccesses, apiTotal, dataPoints, expectedDataPoints) {
        const score = computeExecutionScore(apiSuccesses, apiTotal, dataPoints, expectedDataPoints);
        await this.updateComponent('execution_quality', score, `${apiSuccesses}/${apiTotal} API calls succeeded, ${dataPoints}/${expectedDataPoints} data points collected`);
    }
    /** Update peer review score from QA */
    async recordPeerReview(rating) {
        const score = Math.round(((rating - 1) / 4) * 1000) / 1000; // 1-5 → 0-1
        await this.updateComponent('peer_review', score, `QA Inspector rated ${rating}/5 — ${rating >= 4 ? 'high quality' : rating >= 3 ? 'acceptable' : 'needs improvement'}`);
    }
    /** Load history from Supabase */
    async loadHistory() {
        const count = await getAgentTransactionCount(this.agentDid);
        const score = computeHistoryScore(count);
        await this.updateComponent('history', score, count > 0 ? `${count} past transactions on Agora` : 'New agent — no transaction history');
    }
