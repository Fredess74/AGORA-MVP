...
let webpulse = null;
try {
    webpulse = new WebPulseAgent();
}
catch { /* no PageSpeed key */ }
/** Ensure Supabase has agents seeded */
export async function initSessions() {
    await seedAgentsIfEmpty();
}
/** Start a new demo session */
export async function startSession(query, speedMode = 'fast') {
    const id = uuid();
    const session = {
        id,
        userQuery: query,
...
async function runPipeline(session) {
    ...
    await emit('session_started', id, 'System', 'Demo Started', `Director request: "${session.userQuery}"`, sm);
    // ────────── STEP 2: Task Formulation ──────────
    await emit('task_formulated', id, "Director's AI", 'Formulating task...', 'Analyzing director request and creating structured specification', sm);
    ...
    await emit('task_formulated', id, "Director's AI", 'Task Formulated', `Type: ${task.task_type} Target: ${task.target} Domain: ${task.domain} Deliverables: ${task.deliverables.join(', ')}`, sm, { latencyMs: formulateMs });
    // ────────── STEP 3: MCP Search ──────────
    await emit('mcp_search', id, 'Agora MCP', 'Searching agents...', `search_agents({ capability: "${task.task_type}" })`, sm);
    ...
    await emit('mcp_search', id, 'Agora MCP', `Found ${agents.length} agents`, agents.map(a => `${a.name} [trust: ${a.trustScore.toFixed(3)}, calls: ${a.totalCalls}]`).join('\n'), sm);
    // ────────── STEP 4: Agent Selection ──────────
    await emit('agent_selected', id, 'Procurement AI', 'Evaluating agents...', 'Analyzing capability match, trust scores, and pricing', sm);
    const { agentId, reasoning, latencyMs: selectMs } = await procurement.selectAgent(task, agents);
    session.apiCallsMade++;
    const agent = agents.find(a => a.id === agentId) || agents[0];
    if (!agent) {
        await emit('session_completed', id, 'System', '❌ No Agents Available', 'No agents found on Agora marketplace matching this request.', sm, { error: true });
        return;
    }
    session.selectedAgentId = agent.id;
    session.agentName = agent.name;
    await emit('agent_selected', id, 'Procurement AI', `Selected: ${agent.name}`, reasoning, sm, { agent: { id: agent.id, name: agent.name, did: agent.did, trustScore: agent.trustScore, totalCalls: agent.totalCalls }, latencyMs: selectMs });
    ...
    await trust.checkCapability(task.task_type, task.domain, agent.capabilities, agent.description);
    ...
    if (capComponent && capComponent.score < 0.1) {
        await emit('session_completed', id, 'Agora Trust Engine', '⛔ Operation Rejected — Insufficient Capability Match', `Agent "${agent.name}" capability match is ${(capComponent.score * 100).toFixed(0)}%, below minimum threshold of 10%. No suitable agents available for task type "${task.task_type}".`, sm, { error: true, rejected: true, trustBreakdown: matchBreakdown });
        return;
    }
    session.trustBefore = matchBreakdown;
    // ────────── STEP 5: Negotiation ──────────
    const negotiateStart = Date.now();
    await emit('negotiation', id, 'Account Manager', 'Reviewing task brief...', `Seller AI received task specification for ${task.task_type}`, sm);
    const { response: amResponse, questions, latencyMs: amMs } = await accountManager.negotiate(`Client needs: ${task.task_type} for ${task.target}. Deliverables: ${task.deliverables.join(', ')}`, task);
    session.apiCallsMade++;
    await emit('negotiation', id, 'Account Manager', 'Terms proposed', amResponse, sm, { latencyMs: amMs });
    // Buyer responds to questions
    const { answers, latencyMs: brMs } = await buyerResponse.respond(task, questions);
    session.apiCallsMade++;
    await emit('negotiation', id, 'Procurement AI', 'Clarifications provided', answers.map((a, i) => `${i + 1}. ${a}`).join('\n'), sm, { latencyMs: brMs });
    ...
    await emit('work_started', id, agent.name, 'Starting work...', `Executing ${task.task_type} on ${task.target}`, sm);
    let rawReport = '';
    let apiCalls = 0;
    let dataPoints = 0;
    ...
    // ────────── STEP 7: Delivery ──────────
    await emit('work_completed', id, 'Delivery Manager', 'Formatting report...', 'Packaging raw analysis into executive report format', sm);
    const { report: finalReport, latencyMs: deliveryMs } = await delivery.format(rawReport, task);
    session.apiCallsMade++;
    await emit('work_completed', id, 'Delivery Manager', '📋 Report Delivered', finalReport.slice(0, 500) + (finalReport.length > 500 ? '...' : ''), sm, { latencyMs: deliveryMs, fullReport: finalReport });
    // ────────── STEP 8: QA Review ──────────
    await emit('qa_review', id, 'QA Inspector', 'Reviewing deliverables...', 'Running quality checks on the delivered report', sm);
    ...
    await emit('qa_review', id, 'QA Inspector', `Quality Rating: ${rating}/5`, issues.length > 0 ? `Issues found:\n${issues.map((issue, i) => `${i + 1}. ${issue}`).join('\n')}` : 'All quality checks passed', sm, { rating, issues, latencyMs: qaMs });
    // ────────── STEP 9: Final Trust Summary ──────────
    const finalBreakdown = trust.getBreakdown();
    session.trustAfter = finalBreakdown;
    await emit('trust_updated', id, 'Agora Trust Engine', 'Trust Score Finalized', `${agent.name}: ${finalBreakdown.compositeScore.toFixed(3)} (${finalBreakdown.level})`, sm, { trustBreakdown: finalBreakdown });
    ...
    await emit('session_completed', id, 'System', '✅ Connection Complete', `Task completed successfully. Agent: ${agent.name}, Cost: $${cost.toFixed(2)}, Duration: ${(totalMs / 1000).toFixed(1)}s, Trust: ${finalBreakdown.compositeScore.toFixed(3)}`, sm, {
