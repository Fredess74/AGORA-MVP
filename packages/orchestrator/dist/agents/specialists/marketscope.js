...
Be specific. Cite exact numbers. Never be vague.`);
    ...
    async research(task, onApiCall) {
        ...
        apiCalls++;
        const { data: strategy } = await this.callJSON(`Classify this competitive intelligence topic and plan the research strategy.
Topic: "${topic}"
...
Respond ONLY in JSON.`);
        ...
        try {
            ...
            if (config.githubToken)
                ghHeaders['Authorization'] = `Bearer ${config.githubToken}`;
            const ghRes = await fetch(`https://api.github.com/search/repositories?q=${encodeURIComponent(ghQuery)}&sort=stars&per_page=10`, { headers: ghHeaders });
            if (ghRes.ok) {
                const ghData = await ghRes.json();
                githubRepos = (ghData.items || []).map((r) => ({
                    name: r.full_name,
...
//# sourceMappingURL=marketscope.js.map