...
Be thorough, specific, and never vague. Use the real data.`);
    ...
    async audit(task, onApiCall) {
        const start = Date.now();
        // Parse target repo
        const parsed = parseGitHubRepo(task.target);
        if (!parsed)
            throw new Error(`Cannot parse GitHub repo from: ${task.target}`);
        ...
        const result = await this.call(`Analyze this GitHub repository data and produce a security audit report:\n\n${dataSummary}`);
        return {
